...
<head>
<link rel="stylesheet" href="<c:url value ='/resources/css/readForm2.css'/>">
			...
			deleteBtn.addEventListener("click",function(e){
				e.preventDefault();
					...
					if(confirm("삭제하시겠습니까?")){
	 	 				...
	 	 				location.href = getContextPath()+"/board3/delete2/"+bnum;
					}
				},false);
    }
    //취소(수정모드->읽기모드)
...
<div id="navwrap">
	<nav>
      ...
      <div id = "entirewrap">
       <form:form action="${pageContext.request.contextPath }/board3/modify/" 
					    					 enctype="multipart/form-data"
					    					 method="post"
					    					 modelAttribute="boardVO2">
				...
				<form:hidden path="mapregion"/>
        		
     	<div id = "regid">
      		<div id = "regtext"><b>작성자</b></div>
      		<div id = "regcont">${boardVO2.bid }</div>
      	</div>				
		<div id = "bigbox">
			<div id = "con1">
				<a id ="statusa"href="#">${boardVO2.matchstatus }</a>
			</div>
			<div id = "con2">
				<div id = "date">
					${boardVO2.mdate }
				...
				<div id = "time">
					${boardVO2.stime }~${boardVO2.etime }
				</div>
				<div id = "where">
					${boardVO2.matchregion }
				</div>
			</div>
			<div id = "con3">
				<div id = "how">
...
var mapContainer = document.getElementById('mapdiv'), // 지도를 표시할 div 
    mapOption = { 
        center: new kakao.maps.LatLng(${boardVO2.longitude},${boardVO2.latitude}), // 지도의 중심좌표
        level: 3 // 지도의 확대 레벨
    };
...
// 마커가 표시될 위치입니다 
var markerPosition  = new kakao.maps.LatLng(${boardVO2.longitude},${boardVO2.latitude}); 
...
// marker.setMap(null);    
   		...
   		<div id = "status8">
   			<div id = "statustitle6" class="btn umode" style="float:left; width:80px; padding:15px; border:1px solid #ccc;">용병 수</div>
   			<div id = "statuss6" class="btn umode" style="float:left; width:130px; border:1px solid black; color:white; padding:15px; border:1px solid #ccc;">
					      <form:select class="btn umode" path="wantc" style="width:130px;">
					        <option value="0">=== 선택 ===</option>
									<form:options path="wantc" 
																items="${wantc}"
																itemValue="code" 
																itemLabel="label"/>
					      </form:select></div>
   		...
   		<div id ="comment">				
				...
				<div id = "newcontent" class="row">
        			<div id = "modify" class="row btns">
					        ...
					        <!-- 작성자만 수정, 삭제 가능 -->
					       <c:if test="${sessionScope.member.id == boardVO2.bid }">
					        	<form:button class="btn rmode" id="modifyBtn" style="position:relative; right:30px;">수정</form:button>
					        	<form:button class="btn umode" id="deleteBtn" style="position:relative; right:30px;" data-bnum ="${boardVO2.bnum }">삭제</form:button>
					       </c:if>
					        <!-- 작성자만 수정, 삭제 가능 끝 -->
					        
					        <form:button class="btn umode" style="position:relative; right:30px;" id="cancelBtn">취소</form:button>
					        <form:button class="btn umode" style="position:relative; right:30px;" id="saveBtn">저장</form:button>
        			</div>
